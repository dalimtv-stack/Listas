...
const DEFAULT_M3U_URL = ""; // Sin URL predeterminada, se usará la del usuario o el fallback
...
async function loadM3U(url = DEFAULT_M3U_URL) {
  try {
    ...
    if (!url) {
      console.warn("No URL provided, returning empty channels");
      return [];
    }
    ...
    playlist.items.forEach((item, index) => {
      ...
      console.log(`Procesando: tvg-id=${tvgId}, name=${name}, group_title=${groupTitle}, title=${item.name} (${streamType}), url=${item.url}`);

      const stream = {
        title: `${item.name || name} (${streamType})`, // Título con nombre y tipo, como en 1.2.1
