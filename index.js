...
function extractConfigIdFromPath(requestUrl) {
  ...
  if (requestUrl) {
    try {
      ...
      if (match && match[1]) {
        const configId = match[1]; // UUID, no decode
        console.log('Extracted configId:', configId);
        return configId;
      }
      console.log('No match found in path:', path);
    } catch (err) {
      console.error('Error parsing URL in extractConfigIdFromPath:', err.message, err.stack);
    }
  }
...
// Obtener m3uUrl desde configCache usando configId
function getM3uUrlFromConfigId(configId) {
  if (!configId) return null;
  const m3uUrl = configCache.get(configId);
  if (!m3uUrl) {
    console.log('No m3uUrl found for configId:', configId);
    return null;
  }
  console.log('Retrieved m3uUrl from cache for configId:', configId, m3uUrl);
  return m3uUrl;
}
...
builder.defineStreamHandler(async ({ type, id, url }) => {
 ...
 agus.stderr('Stream error:', error.message, error.stack);
      return { streams: [] };
    }
  }
  return { streams: [] };
...
router.use((req, res, next) => {
  ...
  if (match && match[1]) {
    ...
    console.log('Extracted configId:', req.configId);
    console.log('Modified req.url:', req.url);
  } else {
    console.log('No config prefix matched, passing original URL:', req.url);
  }
  next();
});
