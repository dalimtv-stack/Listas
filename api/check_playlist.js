...
module.exports = async (req, res) => {
  // Solo GET por simplicidad (puedes añadir POST si quieres)
  if (req.method !== 'GET') {
    res.statusCode = 405;
...
<body class="bg-black text-white min-h-screen">
  ...
  <div class="max-w-3xl mx-auto px-6">
    <div class="bg-gray-900/80 backdrop-blur-xl rounded-2xl shadow-2xl p-10 border border-gray-800">
      <form action="/api/playlist" method="GET" class="space-y-6">
        <div>
  ...
  try {
    ...
    // Extraer canales (solo items con url)
    let channels = playlist.items
      .filter(item => item.type === 'track' && item.location?.startsWith('http'))
      .map(item => ({
        name: item.title || item.name || 'Canal sin nombre',
        url: item.location,
        logo: item.tvg?.logo || '',
        group: item.tvg?.group || item.group || 'Sin categoría',
      }));
    ...
    const title = playlist.header?.title || playlist.attrs?.['x-playlist-title'] || 'Lista IPTV';

    let htmlChannels = channels.map(ch => `
...
<body class="bg-black text-white min-h-screen">
  ...
  <div class="max-w-7xl mx-auto px-6 pb-20">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ${htmlChannels || '<p class="text-center col-span-full text-gray-500 text-xl py-20">No se encontraron canales válidos en la lista</p>'}
    </div>

    <div class="mt-12 text-center">
      <a href="/api/playlist" class="text-gray-400 hover:text-white px-6 py-3 border border-gray-700 rounded-xl hover:bg-gray-800 transition">← Nueva lista</a>
...
<body class="bg-black text-white min-h-screen flex items-center justify-center p-6">
  <div class="bg-red-950/60 backdrop-blur-xl border border-red-800 p-10 rounded-2xl max-w-lg text-center">
    ...
    <a href="/api/playlist" class="inline-block bg-red-700 hover:bg-red-600 text-white px-8 py-4 rounded-xl font-medium transition">Intentar otra URL</a>
  </div>
</body>
</html>
    `);
