    ...
    const stream = cloudinary.uploader.upload_stream(
      ...
      (error, result) => {
        if (error) {
          ...
          // lib/upload-to-cloudinary.js - FORMATO STREMIO ULTRA-OPTIMIZADO
          const stremioUrl = cloudinary.url(result.public_id, {
            transformation: [
              { 
                ...
                gravity: "auto"          // ← g_auto (centrado inteligente)
              },
              { quality: "auto" },         // ← q_auto (calidad optimizada)
              { fetch_format: "auto" }     // ← f_auto (formato óptimo)
            ],
            secure: true,
            resource_type: "image"
          });
